...
Secure Coding Chatbot (built by SuperSafe) is a Streamlit web app that acts as an AI-powered security mentor. It helps developers identify and fix hardcoded credentials (API keys, passwords, secrets) in submitted code, then coaches them to remediate using environment variables.
...
- **No persistence, with one exception:** All chat/scan state lives in `st.session_state`. User accounts are the one exception, stored in a local, git-ignored `users.json` (hashed passwords only, never plaintext). On Streamlit Cloud, this file resets on every redeploy, so accounts don't survive deploys. That's an accepted limitation for this demo app, not a bug.
- **Secret masking:** Any detected credentials in user-submitted code must be redacted/masked *before* the code is sent to the Claude API, and before it's shown back in the chat transcript. Never send or display raw secrets.
- **Deployable:** The app must run cleanly on Streamlit Cloud, avoiding local-only dependencies.
